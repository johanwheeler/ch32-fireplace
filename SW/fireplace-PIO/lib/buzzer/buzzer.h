...
const note_tt *curr_song = feliz;

const int note_end = sizeof(feliz) / sizeof(note_tt);
...
void SysTick_Handler(void)
{
    ...
    if (tim_cnt > curr_song[k].duration)
    {
        ...
        else
        {
            ...
            uint16_t per = FUNCONF_SYSTEM_CORE_CLOCK / (freqz[curr_song[k + 1].note - 1] * 4);
